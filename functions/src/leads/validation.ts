...
export function normalizePhone(value: unknown): string {
  if (typeof value !== 'string' || value.length > 24 || !/^[+\d\s().-]+$/.test(value)) throw new LeadError('INVALID_PHONE');
  let digits = value.replace(/\D/g, '');
  if (digits.length === 12 || digits.length === 13) {
    if (!digits.startsWith('55')) throw new LeadError('INVALID_PHONE');
    digits = digits.slice(2);
  }
  if (!ddds.has(digits.slice(0, 2))) throw new LeadError('INVALID_PHONE');
  const local = digits.slice(2);
  if (!(/^[2-5]\d{7}$/.test(local) || /^9\d{8}$/.test(local)) || /^(\d)\1+$/.test(local)) throw new LeadError('INVALID_PHONE');
  return `55${digits}`;
}
function text(data: Record<string, unknown>, key: string, max: number, required = false): string {
  const value = data[key];
  if (value === undefined && !required) return '';
  if (typeof value !== 'string' || value.length > max || /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(value)) throw new LeadError('INVALID_PAYLOAD');
  const clean = value.trim();
  if (required && !clean) throw new LeadError('INVALID_PAYLOAD');
  return clean;
}
function referrer(value: string): string {
  if (!value) return '';
  try { const url = new URL(value); if (!['https:', 'http:'].includes(url.protocol)) throw new Error(); return url.origin + url.pathname; }
  catch { throw new LeadError('INVALID_REFERRER'); }
}
export function validateLead(input: unknown, expectedType?: LeadType): LeadPayload {
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new LeadError('INVALID_PAYLOAD');
  const data = input as Record<string, unknown>;
  if (text(data, 'website', 200)) throw new LeadError('SUBMISSION_REJECTED'); // Honeypot, never persisted.
  const requestId = text(data, 'requestId', 36, true);
  if (!/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/i.test(requestId)) throw new LeadError('INVALID_REQUEST_ID');
  const leadType = data.leadType;
  if ((leadType !== 'QUIZ' && leadType !== 'CONTACT') || (expectedType && expectedType !== leadType)) throw new LeadError('INVALID_LEAD_TYPE');
  const name = text(data, 'name', 100, true).replace(/\s+/g, ' ');
  if (name.length < 2 || !/\p{L}/u.test(name)) throw new LeadError('INVALID_NAME');
  const email = text(data, 'email', 254);
  if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new LeadError('INVALID_EMAIL');
  const page = text(data, 'page', 300);
  if (page && (!page.startsWith('/') || /[?#]/.test(page))) throw new LeadError('INVALID_PAGE');
  ...
  const preferredModalities = Array.isArray(data.preferredModalities)
    ? data.preferredModalities.filter((item): item is string => typeof item === 'string').map(s => s.slice(0, 50)).slice(0, 15)
    : undefined;
  const experience = text(data, 'experience', 50) || undefined;
  const availability = Array.isArray(data.availability)
    ? data.availability.filter((item): item is string => typeof item === 'string').map(s => s.slice(0, 50)).slice(0, 10)
    : undefined;
  const objective = text(data, 'objective', 100) || undefined;
  const recommendedClassIds = Array.isArray(data.recommendedClassIds)
    ? data.recommendedClassIds.filter((item): item is string => typeof item === 'string').map(s => s.slice(0, 80)).slice(0, 10)
    : undefined;
  ...
  if (leadType === 'QUIZ') {
    ...
    if (answers && typeof answers === 'object' && !Array.isArray(answers)) {
      ...
      if (!entries.length && !recommendedClassIds && age === undefined) throw new LeadError('INVALID_QUIZ');
      if (entries.length > 20) throw new LeadError('INVALID_QUIZ');
      for (const [key, value] of entries.sort(([a], [b]) => a.localeCompare(b))) {
        if (!/^[a-zA-Z][a-zA-Z0-9_-]{0,39}$/.test(key) || ['__proto__','constructor','prototype'].includes(key) || typeof value !== 'number' || !Number.isInteger(value) || value < 0 || value > 100) throw new LeadError('INVALID_QUIZ');
  ...
  return { requestId, name, phone: normalizePhone(data.phone), leadType, email,
    ...
    ...(objective ? { objective } : {}),
    ...(recommendedClassIds ? { recommendedClassIds } : {}),
